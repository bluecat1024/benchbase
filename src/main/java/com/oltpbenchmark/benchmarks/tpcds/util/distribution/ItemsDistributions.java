...
import static com.oltpbenchmark.benchmarks.tpcds.util.distribution.IntValuesDistribution.buildIntValuesDistribution;
...
public final class ItemsDistributions
{
    ...
    public static List<Integer> pickRandomManagerIdRange(IdWeights idWeights, RowRandomInt stream)
    {
        int index = ITEM_MANAGER_ID_DISTRIBUTION.pickRandomIndex(idWeights.ordinal(), stream);
        return List.of(ITEM_MANAGER_ID_DISTRIBUTION.getValueAtIndex(1, index), ITEM_MANAGER_ID_DISTRIBUTION.getValueAtIndex(2, index));
    }

    public static List<Integer> pickRandomManufactIdRange(IdWeights idWeights, RowRandomInt stream)
    {
        int index = ITEM_MANUFACT_ID_DISTRIBUTION.pickRandomIndex(idWeights.ordinal(), stream);
        return List.of(ITEM_MANUFACT_ID_DISTRIBUTION.getValueAtIndex(1, index), ITEM_MANUFACT_ID_DISTRIBUTION.getValueAtIndex(2, index));
    }

    public static String pickRandomSize(SizeWeights sizeWeights, RowRandomInt stream)
    {
        return SIZES_DISTRIBUTION.pickRandomValue(0, sizeWeights.ordinal(), stream);
    }

    public static String pickRandomColor(ColorsWeights colorsWeights, RowRandomInt stream)
    {
        return COLORS_DISTRIBUTION.pickRandomValue(0, colorsWeights.ordinal(), stream);
    }

    public static String pickRandomUnit(RowRandomInt stream)
    {
